...
public class TypingManager : MonoBehaviour
{
    ...
    // Oculus 컨트롤러 트리거 입력을 받기 위한 변수 추가
    ...
    private void Update()
    {
        ...
        if (OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger)&& Vector3.Distance(player.position, gretel.position)<3f)
        {
            
            if (!triggerPressed)
            {
                GetInputDown(); // 트리거 버튼이 처음으로 눌렸을 때 GetInputDown() 호출
                triggerPressed = true;
            }
        }
        else
        {
            if (triggerPressed)
            {
                GetInputUp(); // 트리거 버튼이 놓여졌을 때 GetInputUp() 호출
                triggerPressed = false;
            }
        }
    }

    public void Typing(string[] dialogs, Text textObj)
    {
        isDialogEnd = false;
        dialogsSave = dialogs;
        tmpSave = textObj;
    ...
    IEnumerator Typer(char[] chars, Text textObj)
    {
        int currentChar = 0;
